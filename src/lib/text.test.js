...
test('fullName(user) should get correct result', (t) => {
  ...
  t.is(
    text.fullName({ first_name: 'Foo', username: 'foobar' }),
    'Foo (@foobar)',
  );
  t.is(
    text.fullName({ last_name: 'Bar', username: 'foobar' }),
    'Bar (@foobar)',
  );
  t.is(
    text.fullName({ first_name: 'Foo', last_name: 'Bar', username: 'foobar' }),
    'Foo Bar (@foobar)',
  );
});
...
test('fullNameId(user) should get correct result', (t) => {
  ...
  t.is(
    text.fullNameId({ id: 2, first_name: 'Foo', last_name: 'Bar' }),
    'Foo Bar #2',
  );
  t.is(
    text.fullNameId({ id: 2, first_name: 'Foo', username: 'foobar' }),
    'Foo (@foobar) #2',
  );
  t.is(
    text.fullNameId({ id: 2, last_name: 'Bar', username: 'foobar' }),
    'Bar (@foobar) #2',
  );
  t.is(
    text.fullNameId({
...
test('chatTitle(chat) should return correct name', (t) => {
  ...
  t.is(
    text.chatTitle({ type: 'private', id: 1, title: 'Chat', username: 'chat' }),
    'Chat (@chat)',
  );
});
